...
# fan and fan0 come in from fanplot so they can be commented out here...
...
norm.density.palette <- function(sds = 3, colmax = "tomato", colmin = "transparent", gamma = 1, scale = 1) {
  rgbmax <- col2rgb(colmax, alpha = TRUE)
  rgbmin <- c(col2rgb(colmin, alpha = FALSE), 0)
  if (gamma <= 0) 
    stop("gamma must be greater than 0")
  return(
    # Return a generator of n colours
    function(n) {
...
get_percentiles <- function(n = 1, sds = 3) {
  if(n == 0) {
    #return (c(0.004431848, 0.053990967, 0.241970725, 0.758029275, 0.946009033, 0.995568152))
    return (c(0.025, 0.2, 0.35, 0.65, 0.8, 0.975))
    #return (c(0.0001338302, 0.0044318484, 0.0539909665, 0.2419707245, 0.7580292755, 0.9460090335, 0.9955681516, 0.9998661698))
  }
  else {
    m <- 2*n*sds+1
    x <- seq(-sds, sds, length = m)[1:((m-1)/2)]
    print(x)
...
shinyServer(function(input, output, session) {
  ...
  renderPlotWithParameters <- function(plot_type) {
    ...
    smoothing <- 1
    if(plot_type == "coarse")
      pps = get_percentiles(n = 0) # n=0 selects #c(0.025, 0.15, 0.30, 0.70, 0.85, 0.975)
    else 
    ...
    plot((original_val[mid,] + original_val[mid+1,])/2,
         type = if (plot_type == "basic") "p" else "l", 
         pch = 18,
    ...
    if (plot_type == "coarse" || plot_type == "detailed") {
      fan0(smoothed_val,
           ...
           n.fan = if (plot_type == "coarse") 5 else NULL,   # this adjusts the low density nicely. Bigger = denser
           xlab = input$tLabel,
           ylab = input$modeLabel,
           main = input$mainTitle,
           xaxt = "n"
